...
def generate_graph(
    ...
    for term_id in term_ids:
        ...
        graph.add_node(term_id, label=clean_term(term))
    ...
    for rel_id in relationship_ids:
        ...
        graph.add_edge(subj_id, obj_id, label=pred, pred_id=rel_id)

    return graph


if __name__ == "__main__":
    diagram_headers = get_diagram_headers(INPUT_PATH)
    elements = parse_elements(INPUT_PATH)
    term_ids, rel_ids = extract_elements(elements)
    rank_terms = ["rdfs:subClassOf", "rdf:type"]
    graph = generate_graph(elements, term_ids, rel_ids, rank_terms)
